...
void Analogies::Execute () {
	try {
        result_ = wrapper_->analogies( words_ , k_ );
...
// 错误回调
void Analogies::HandleErrorCallback () {
...
// 成功回调
void Analogies::HandleOKCallback () {
    ...
    for(uint32_t i = 0; i < result_.size(); i++) {
        ...
        returnObject->Set(
            Nan::New<v8::String>("word").ToLocalChecked(),
